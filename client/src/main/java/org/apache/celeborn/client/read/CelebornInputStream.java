...
public abstract class CelebornInputStream extends InputStream {
  ...
  private static final class CelebornInputStreamImpl extends CelebornInputStream {
    ...
    public int read(byte[] b, int off, int len) throws IOException {
      ...
      while (readBytes < len) {
        while (position >= limit) {
          if (!fillBuffer()) {
            if (readBytes <= 0) {
              logger.error("expected size {}, actual read bytes {}", len, readBytes);
            }
    ...
    private boolean moveToNextChunk() throws IOException {
      ...
      currentChunk = null;
      logger.error(
          "currentChunk is set to null in moveToNextChunk, shuffleKey {}, attemptNumber {}, startMapIndex {}, endMapIndex {}",
          shuffleKey,
          attemptNumber,
          startMapIndex,
          endMapIndex);
    ...
    private boolean fillBuffer() throws IOException {
      if (currentChunk == null) {
        logger.error(
            "fillBuffer returns false because currentChunk is null, shuffleKey {}, attemptNumber {}, startMapIndex {}, endMapIndex {}",
            shuffleKey,
            attemptNumber,
            startMapIndex,
            endMapIndex);
      ...
      while (currentChunk.isReadable() || moveToNextChunk()) {
        ...
        if (attemptId == attempts[mapId]) {
          ...
          logger.error(
              "succeeded attempt id {}, current attempt id {}", attempts[mapId], attemptId);
        }
      }
